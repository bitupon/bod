...
Kiiro.BOD.JSOMUtil = (function () {
    ...
    var _updateSite = function (title, desc, url, success, failure, param) {
    ...
    var _deleteSite = function (url, success, failure, param) {
        var context = new SP.ClientContext(url);
        var web = context.get_web();
        web.deleteObject();
    ...
    var _createList = function (url, listName, listTemplate, contentTypeId, oldContentTypeName) {
        return $.Deferred(function (dfd) {
            var context = new SP.ClientContext(url);
            var web = context.get_web();
                ...
                Function.createDelegate(this, function () {
                    ...
                    context.executeQueryAsync(
                        Function.createDelegate(this, function () {
                            ...
                            context.executeQueryAsync(
                                Function.createDelegate(this, function () {
                                    if (oldContentTypeName) {
                                        var ctColl = newList.get_contentTypes();
                                        context.load(ctColl);
                                        context.executeQueryAsync(
                                            Function.createDelegate(this, function () {
                                                var ctEnumerator = ctColl.getEnumerator(),
                                                    ctCurrent,
                                                    ctName,
                                                    ctid;
                                                while (ctEnumerator.moveNext()) {
                                                    ctCurrent = ctEnumerator.get_current();
                                                    ctName = ctCurrent.get_name();
                                                    ctid = ctCurrent.get_id();
                                                    if (ctName == oldContentTypeName) {
                                                        ctCurrent.deleteObject();
                                                        context.executeQueryAsync(
                                                            Function.createDelegate(this, function () {
                                                                dfd.resolve();
                                                            }),
    ...
    var _addWebPart = function (url, webPartPath, pagePath, zoneName, zoneId, param) {
        return $.Deferred(function (dfd) {
            var context = new SP.ClientContext(url);
            var web = context.get_web();
            ...
            context.executeQueryAsync(
                Function.createDelegate(this, function () {
                    ...
                    .done(Function.createDelegate(this, function (data) {
                        var webPartXml = data;
                        if (param) {
                            if (param.SiteRelativeUrl) {
                                webPartXml = webPartXml.replace(/Site-Relative--Url/g, param.SiteRelativeUrl);
                            }
                            if (param.SiteAbsoluteUrl) {
                                webPartXml = webPartXml.replace(/Site-Absolute--Url/g, param.SiteAbsoluteUrl);
                            }
                            if (param.AbsoluteUrl) {
                                webPartXml = webPartXml.replace(/Absolute--Url/g, param.AbsoluteUrl);
                            }
                            if (param.RelativeUrl) {
                                webPartXml = webPartXml.replace(/Relative--Url/g, param.RelativeUrl);
                            }
                            if (param.ListGuid) {
                                webPartXml = webPartXml.replace(/List--Guid/g, param.ListGuid);
                            }
                        }

                        var file = newWeb.getFileByServerRelativeUrl(pagePath);
